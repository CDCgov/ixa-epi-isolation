...
ixa_sir_final_sizes <- ixa_sir_df |> filter(InfectionStatus == "Recovered" & 
t == max(t))

epidemic_sims <- ixa_sir_final_sizes |> 
filter(count > (sim_pop_size * simple_sir_final_size / 2)) |> pull(simulation)
...
ixa_sir_epidemic_df |>
  ...
  geom_line(
    data = ixa_sir_epidemic_summary,
    aes(x = t, y = lower_95_ci, color = InfectionStatus), lty = 3, lwd = 0.5
  ) +
  geom_line(
    data = ixa_sir_epidemic_summary,
    aes(x = t, y = upper_95_ci, color = InfectionStatus), lty = 3, lwd = 0.5
  ) +
