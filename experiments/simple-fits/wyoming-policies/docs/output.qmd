...
author: "Will Koval (AD71@cdc.gov, CDC/ORR/CFA), Erik Rosenstrom (XUI6@cdc.gov, CDC/ORR/CFA)"
format: pdf
toc: true
...
| `PreviousIsolationGuidance.overall_policy_duration` | The minimum policy duration from symptom onset if the individuals has a positive test results | 
| `PreviousIsolationGuidance.mild_symptom_isolation_duration` | The minimum required duration of isolation for individuals with mild symptoms | 
| `PreviousIsolationGuidance.moderate_symptom_isolation_duration` | The minimum required duration of isolation for individuals with moderate symptoms | 
| `PreviousIsolationGuidance.delay_to_retest` | The delay between an individual's first negative test and the subsequent retest | 
| `PreviousIsolationGuidance.policy_adherence` | Probability an individual follows the isolation guidance policy conditional on symptom duration > isolation delay period | 
| `PreviousIsolationGuidance.isolation_delay_period` | Delay from symptom onset to when an individual starts following the isolation guidance policy |
| `PreviousIsolationGuidance.test_sensitivity` | Sensitivity of the test used to determine if an individual is infected |  
...
We assume that individuals follow the updated or previous isolation guidance policy during the calibration window. Under this policy symptomatic 
individuals are tested and isolate for specified duration depending on their symptom severity. In some cases, they follow post-
isolation precautions. In our model we define post-isolation precautions as mask wearing.

| Parameter | Value | Source |
| :-------: | :---: | ------ |
| `PreviousIsolationGuidance.overall_policy_duration` | 10 days | CDC Isolation Guidance |
| `PreviousIsolationGuidance.mild_symptom_isolation_duration` | 5 days | CDC Isolation Guidance |
| `PreviousIsolationGuidance.moderate_symptom_isolation_duration` | 10 days | CDC Isolation Guidance | 
| `PreviousIsolationGuidance.delay_to_retest` | 2 days | CDC Isolation Guidance |
| `PreviousIsolationGuidance.policy_adherence` | 0.7 | Delphi COVIDCast Masking Survey Data |
| `PreviousIsolationGuidance.isolation_delay_period` | 0 days | Individual level isolation guidance work |
| `PreviousIsolationGuidance.test_sensitivity` | 0.7 | Wolfl-Duchek et al. (2022) Mircobiol Spectr. Jegerlehner et al. (2021) IJID |
...
max_target = experiment.tolerance_dict[steps - 2]
