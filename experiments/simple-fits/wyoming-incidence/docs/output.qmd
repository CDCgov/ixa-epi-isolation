...
| :-------: | :---: | ------ |
| $\alpha_\text{Home}$ | Uniform(0.0, 0.2) | Assumed/uninformative |
| $\alpha_\text{Work}$ | Uniform(0.0, 0.2) | Assumed/uninformative |
| $\alpha_\text{Sch}$ | Uniform(0.0, 0.2) | Assumed/uninformative |
| `initial_recovered` | Beta(5, 15) | Assumed partial protection from infections in spring/summer before calibration |
| `infectiousness_rate_fn.scale` | Uniform(0.0, 0.2) | Assumed/uninformative |
...
The following plot shows the priors (light-shaded histograms) and posteriors (dark-shaded histograms) for the 6 parameters fitted in the calibration process.
```{python}
#| echo: false
#| fig-width: 5
#| fig-height: 6
...
g = sns.displot(
    data=input_data_long_prior_post, x="value", col="parameter", hue="step",
...
g3 = sns.displot(
    data=input_data_long, x="value",
    col=facet_by[0], row=facet_by[1],
    kind="hist", height = 1, aspect = 1,
    facet_kws={"sharex": sharex, "sharey": True},
    fill=True, stat="density", binwidth = 0.01, common_bins=False
)
...
The scenario analysis assesses the effects of the update and previous isolation guidance on hospitalizations in the calibrated "No Guidance" model. The table below shows the counterfactual scenarios and the relevant parameters varied by scenario.

| Guidance Policy | Policy Adherence | Facemask Efficacy |
| :-------: | :---: | ------ |
| No Guidance | 0% | 0% |
| Updated | 25% | 20% |
| Updated | 25% | 50% |
...
We sample 50 particles from the approximate posterior to combine with each counterfactual scenario parameter set. The results are presented in the following figure, which shows the weekly incidence hospital admission over time. The figure is faceted by policy adherence and facemask efficacy. The calibration target data is presented to show the robustness of the calibration (shown in the "No Guidance" scenario) to stochasticity.
...
experiment = Experiment(
    img_file="../data/experiment_history.pkl",
    azure_batch = False,
...
for scenario in os.listdir(scenarios_dir):
    ...
    for key in raw_griddle["parameters"].keys():
        flattened_dict = utils.flatten_dict(input_params)
        for k in flattened_dict.keys():
            if "UpdatedIsolationGuidance" in k:
                adherence = flattened_dict["epi_isolation.GlobalParams>>>guidance_policy>>>UpdatedIsolationGuidance>>>policy_adherence"]
                if adherence == 0:
                ...
                adherence = flattened_dict["epi_isolation.GlobalParams>>>guidance_policy>>>PreviousIsolationGuidance>>>policy_adherence"]
                parameter_dict["Guidance"] = "Previous"
                parameter_dict["Policy Adherence"] = adherence
                break
            elif "facemask" in k:
                parameter_dict["Facemask Efficacy"] = flattened_dict["epi_isolation.GlobalParams>>>facemask_parameters>>>facemask_efficacy"]
...
target_data = pl.read_csv(
"../input/target_data.csv"
)

# Create subplots for each level of Policy Adherence
policy_adherence_levels = scenario_data["Policy Adherence"].unique().to_list()
policy_adherence_levels.remove(0)
policy_adherence_levels.sort()
num_levels = len(policy_adherence_levels)

calibration_data = scenario_data.filter(pl.col("Policy Adherence") == 0)
modified_scenario_data = scenario_data.filter(pl.col("Policy Adherence") != 0)

modified_calibration_data = []
for level in policy_adherence_levels:
    temp_df = calibration_data.with_columns(
        pl.lit(level).alias("Policy Adherence")
    )
    modified_calibration_data.append(temp_df)
calibration_data = pl.concat(modified_calibration_data)
modifed_scenario_data = pl.concat([calibration_data, modified_scenario_data])

scenario_data = scenario_data.sort("Guidance")
modifed_scenario_data = modifed_scenario_data.sort("Guidance")


plot = sns.relplot(
    modifed_scenario_data,
    x="t_upper",
    y="count",
    hue="Guidance",
    col="Policy Adherence",
    row="Facemask Efficacy",
    kind="line",
    estimator="median",
...
for x in range(ax.shape[0]):
    for y in range(ax.shape[1]):
        current_ax = ax[x, y]
        sns.scatterplot(target_data, x="t", y="total_admissions", ax = current_ax, zorder=10, label = "Target Data", color="black", marker="x", legend=False)

plot.set_axis_labels("Time (days)", "Incident weekly hospital admissions")

plt.show()
```

The next two figures show the distribution of peak and cumulative hospital admissions for each scenario. The figure is faceted by policy adherence and facemask efficacy.

```{python}
#| echo: false
#| fig-width: 6.5
#| fig-height: 6.5
modifed_scenario_data = modifed_scenario_data.with_columns(
    pl.col("count").max().over(["scenario", "Policy Adherence", "Facemask Efficacy"]).alias("max_count")
)

modifed_scenario_data = modifed_scenario_data.with_columns(
    pl.col("count").sum().over(["scenario", "Policy Adherence", "Facemask Efficacy"]).alias("total_count")
)
...
plt.setp(plt.gcf().axes, xlabel="Peak Weekly Hospital Admissions")
plt.show()
...
plt.setp(plt.gcf().axes, xlabel="Total Hospital Admissions")
plt.show()
```
