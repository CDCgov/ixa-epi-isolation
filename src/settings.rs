...
pub trait SettingType {
    fn calculate_multiplier(&self, context: &Context, setting_id: usize) -> f64;
}
...
impl SettingType for Home {
    ...
    fn calculate_multiplier(&self, context: &Context, setting_id: usize) -> f64 {
        let n_members = context
            .get_setting_members::<Self>(setting_id)
            .unwrap()
            .len();
        let setting_properties = context.get_setting_properties::<Self>();
...
pub trait ContextSettingExt {
    ...
    // TODO make internal
    fn register_setting_type<T: SettingType + 'static>(&mut self, setting_props: SettingProperties);
    // TODO replace with add itinerary
    fn add_itinerary(&mut self, person_id: PersonId, itinerary: Vec<ItineraryEntry>);
    fn add_setting<T: SettingType + 'static>(
        &mut self,
        setting_id: usize,
        person_id: PersonId,
    ) -> Result<(), IxaError>;
    fn register_setting_for_person<T: SettingType + 'static>(
        &mut self,
        setting_id: usize,
        person_id: PersonId,
    ) -> Result<(), IxaError>;
...
impl ContextSettingExt for Context {
    ...
    fn register_setting_for_person<T: SettingType + 'static>(
        ...
        match self
            .get_data_container_mut(SettingDataPlugin)
            .members_to_settings
            .entry(person_id)
        {
            Entry::Vacant(entry) => {
            ...
            Entry::Occupied(mut entry) => {
                ...
                match entry.get_mut().entry(TypeId::of::<T>()) {
                    ...
                    Entry::Occupied(mut setting_map) => {
                        setting_map.get_mut().insert(setting_id);
                    }
                }
                Ok(())
            }
    ...
    fn add_itinerary(&mut self, person_id: PersonId, itinerary: Vec<ItineraryEntry>) {
        ...
        // TODO add people as members, iterate through each entry
            ...
            .entry(TypeId::of::<T>())
        {
            ...
            Entry::Occupied(mut entry) => {
                ...
                match entry.get_mut().entry(setting_id) {
                    Entry::Vacant(setting_map) => {
                        setting_map.insert(vec![person_id]);
                    }
                    Entry::Occupied(mut setting_map) => {
                        setting_map.get_mut().push(person_id);
                    }
                }
                self.register_setting_for_person::<T>(setting_id, person_id)?;
                Ok(())
    ...
    fn calculate_total_infectiousness_multiplier_for_person(
        ...
        if let Some(itinerary) = self.get_itinerary(person_id) {
            ...
            for entry in itinerary {
                total_infectiousness += entry
                    .setting_type
                    .calculate_multiplier(self, entry.setting_id);
            }
...
mod test {
    ...
    fn test_setting_type_creation() {
        ...
        println!(
            "test_setting_type_creation:: Creating  house type with alpha {}",
            home_props.alpha
        );
        println!(
            "test_setting_type_creation:: Creating censustract type with alpha {}",
            tract_props.alpha
        );
    ...
    fn test_add_itinterary() {
        ...
        context.add_itinerary(person, itinerary);

        // TODO test to make sure people were added as members
    }
    ...
    fn test_setting_registration() {
        ...
        for s in 0..5 {
            ...
            println!(
                "Members of house {s} are {:#?} - CensusTract {:#?}",
                members, tract_members
            );
        }
    }
    ...
    fn test_setting_multiplier() {
        ...
        let inf_multiplier =
            context.calculate_total_infectiousness_multiplier_for_setting::<Home>(home_id);
        let members = context.get_setting_members::<Home>(home_id).unwrap();
        println!(
            "Setting multiplier {inf_multiplier} with members  {:#?}",
            members
        );
    ...
    fn test_person_settings() {
        ...
        println!(
            "Person settings for {person} {:#?}",
            person_settings.get(&TypeId::of::<Home>())
        );
        println!("Settings for person {person}: {:#?}", person_settings);

        assert_eq!(
            home_ids,
            person_settings.get(&TypeId::of::<Home>()).unwrap().clone()
        );
        assert_eq!(
            tract_ids,
            person_settings
    ...
    fn test_total_infectiousness_multiplier() {
        ...
        assert_eq!(
            ...
            ((6.0 - 1.0) as f64).powf(0.1) + ((6.0 - 1.0) as f64).powf(0.01)
        );

        let _ = context.add_setting::<CensusTract>(1, person);
        let inf_multiplier_three_settings = context
            .calculate_total_infectiousness_multiplier_for_person(person)
            .unwrap();
        ...
        assert_eq!(
            inf_multiplier_three_settings,
            ((6.0 - 1.0) as f64).powf(0.1)
                + ((6.0 - 1.0) as f64).powf(0.01)
                + ((4.0 - 1.0) as f64).powf(0.01)
        );
    }
