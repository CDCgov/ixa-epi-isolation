...
use crate::interventions::ContextTransmissionModifierExt;
...
pub trait ContextSettingExt {
    ...
    /// Preliminary method to calculate the effective infectiousness multiplier
    /// Only supplies a modifier generated by current transmission modifiers acting on `person_id`
    /// Then multiplies by the total infectiousness multiplier. This assumes that transmission modifiers
    /// of total infectiousness are independent of the setting type and are linear
    fn calculate_effective_infectiousness_multiplier_for_person(&self, person_id: PersonId) -> f64;
...
impl ContextSettingExt for Context {
    ...
    fn calculate_effective_infectiousness_multiplier_for_person(&self, person_id: PersonId) -> f64 {
        let modifier = self.get_modified_relative_total_transmission_person(person_id);
        modifier * self.calculate_total_infectiousness_multiplier_for_person(person_id)
    }
