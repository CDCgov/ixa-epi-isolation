use ixa::{
    define_person_property_with_default, trace, Context, ContextPeopleExt, ContextRandomExt,
...
define_person_property_with_default!(LastTestStatus, LastTest, LastTest::None);
...
fn event_subscriptions(context: &mut Context) {
    ...
    context.subscribe_to_event::<PersonPropertyChangeEvent<LastTestStatus>>(
        move |context, event| {
            ...
            match event.current {
                LastTest::Positive => {
                    let symptom_status = context.get_person_property(event.person_id, Symptoms);
                    match symptom_status {
                        Some(SymptomValue::Category1) | Some(SymptomValue::Category2) => {
                            let isolation_duration = symptom_duration.max(5.0);
                            context.set_person_property(
                                event.person_id,
                                IsolatingStatus,
                                Isolating::Participating,
                            );
                            context.add_plan(t + islation_duration, move |context| {
                                context.set_person_property(
                                    event.person_id,
                                    IsolatingStatus,
                                    Isolating::None,
                        ...
                        Some(SymptomValue::Category3) | Some(SymptomValue::Category4) => {
                            let isolation_duration = symptom_duration.max(10.0);
                            context.set_person_property(
                                event.person_id,
                                IsolatingStatus,
                                Isolating::Participating,
                            ...
                            context.add_plan(t + islation_duration, move |context| {
                                ...
                                trace!("Person {} is now not isolating", event.person_id);
                            });
                            trace!("Person {} is now isolating", event.person_id);
                        }
                        _ => (),
    ...
    context.subscribe_to_event::<PersonPropertyChangeEvent<MaskingStatus>>(
        move |context, event| {
            let t = context.get_current_time();
            let symptom_onset = context.get_person_property(event.person_id, Symptoms);
            match event.current {
                Masking::Wearing => {
