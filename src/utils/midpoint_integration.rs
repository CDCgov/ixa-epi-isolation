...
pub fn midpoint_integration(x: &[f64], y: &[f64]) -> Result<f64, IxaError> {
    if x.len() != y.len() {
...
pub fn cumulative_integral(x: &[f64], y: &[f64]) -> Result<Vec<f64>, IxaError> {
    ...
    Ok(x.windows(2)
        ...
        .scan(0.0, |state, (ts, rates)| {
            *state += midpoint_integration(ts, rates).unwrap();
            Some(*state)
        })
        .collect())
}
...
mod tests {
    ...
    fn test_midpoint_integration_simple() {
        let x = vec![0.0, 1.0];
        let y = vec![0.0, 1.0];
        assert_almost_eq!(midpoint_integration(&x, &y).unwrap(), 0.5, 0.0);
    }
    ...
    fn test_midpoint_integration_vector() {
        ...
        assert_almost_eq!(midpoint_integration(&x, &y).unwrap(), 21.5, 0.0);
    }

    #[test]
    fn test_midpoint_integral_x_y_len() {
        let x = vec![1.0, 2.0, 3.0, 4.0];
        let y = vec![1.0, 2.0];

        let e = midpoint_integration(&x, &y).err();
        match e {
