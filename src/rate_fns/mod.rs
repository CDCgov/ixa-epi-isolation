...
pub trait InfectiousnessRateFn {
    ...
    fn rate(&self, t: f64) -> f64 {
        self.rate_with_scale_offset(t, 1.0, 0.0)
    }
    fn rate_with_scale_offset(&self, t: f64, scale: f64, offset: f64) -> f64;
    /// Returns the time at which a person is no longer infectious
    fn max_time(&self) -> f64;
    /// Returns a time given a probability
    fn inverse_cum(&self, p: f64) -> Option<f64> {
        self.inverse_cum_with_scale_offset(p, 1.0, 0.0)
    }
...
impl<T: ?Sized + InfectiousnessRateFn> InfectiousnessRateFn for ScaledRateFn<'_, T> {
    fn rate(&self, t: f64) -> f64 {
        self.base.rate_with_scale_offset(t, self.scale, self.offset)
    }
    fn rate_with_scale_offset(&self, t: f64, scale: f64, offset: f64) -> f64 {
        self.base
            .rate_with_scale_offset(t, scale * self.scale, offset)
    }
    fn inverse_cum(&self, p: f64) -> Option<f64> {
        self.base
            .inverse_cum_with_scale_offset(p, self.scale, self.offset)
    }
    fn inverse_cum_with_scale_offset(&self, p: f64, scale: f64, offset: f64) -> Option<f64> {
        self.base
            .inverse_cum_with_scale_offset(p, scale * self.scale, offset)
    }
    fn max_time(&self) -> f64 {
        self.base.max_time()
    }
