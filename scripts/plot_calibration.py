...
results = results.with_columns(
    pl.col("simulation").map_elements(lambda x: experiment.step_from_index(x), return_dtype=pl.Int32).alias("step")
)
...
for i, step in enumerate([0, 2]):
    ...
    for simulation in step_results["simulation"].unique():
        sim_data = step_results.filter(pl.col("simulation") == simulation).sort("t")
        ax.plot(sim_data["t"], sim_data["result_count"], alpha=0.05, color="blue")
    ...
    ax_hist = axes[i + 2]
    step_distances = distances.filter((pl.col("step") == step) & (pl.col("distance") < 200))
